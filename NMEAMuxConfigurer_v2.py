...
class SerialApp:
    def __init__(self, root):
        ...
        # 4. Пустая секция (теперь содержит дополнительные элементы)
        frame4 = tk.Frame(root, padx=10, pady=5)
        frame4.pack(fill="both", expand=True)

        # 4.1 Кнопки "Канал 1" – "Канал 8"
        self.channel_frame = tk.Frame(frame4)
        ...
        self.config_frame = tk.Frame(frame4)
        ...
        # 4.3 Список NMEA 0183 сообщений с чекбоксами
        self.nmea_frame = tk.Frame(frame4)
        self.nmea_frame.pack(fill="both", expand=True)
        ...
        for sentence in self.nmea_sentences:
            var = tk.BooleanVar()
            self.nmea_vars.append(var)
            chk = tk.Checkbutton(self.nmea_scrollable_frame, text=sentence, variable=var)
            chk.pack(anchor="w")
